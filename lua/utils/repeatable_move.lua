-- NOTE: Copied from treesitter-textobjects
-- Ref: https://github.com/nvim-treesitter/nvim-treesitter-textobjects/blob/master/lua/nvim-treesitter/textobjects/repeatable_move.lua
-- With this we remove two dependencies (treesitter and treesitter-textobjects) and we avoid the long name error (ENAMETOOLONG)
-- when sourcing the script without quicky workarouns like rename the directory repo.
...
-- NOTE: Prefer to define commands manually, so we can include descriptions
-- and references in lsp work without issues
-- M.commands = {
