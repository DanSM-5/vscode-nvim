...
local ns = vim.api.nvim_create_namespace('pack_ui') -- highlight namespace
...
--   local plugin_dir = ('%s/plugin'):format(config)
--   local after_plugin_dir = ('%s/after/plugin'):format(config)
--   local init_file = ('%s/init.lua'):format(config)
--
...
local function build_content()
  ...
  if state.show_help then
    ...
    add("   <lleader>l   Open the plugin git's history", 'PackUiHelp')
    add('   <lleader>t   Open terminal in plugins path', 'PackUiHelp')
    add('   <lleader>i   Inspect plugin data', 'PackUiHelp')
  end
...
local function setup_keymaps()
  ...
  vim.keymap.set('n', 'X', function()
    ...
    local msg =
      string.format('Remove %d non-active plugin(s) not in config?\n\n%s', #to_clean, table.concat(to_clean, '\n'))
  ...
  vim.keymap.set('n', 'D', function()
    ...
    if choice == 1 then
      ...
      if del_ok then
        ...
        local loaded = pack.load_tbl[name]
        if loaded then
          pack.load_tbl[name] = nil -- remove from load_tbl
          pcall(pack.load_tbl[name].clear) -- clear any pending autocmd
        end
